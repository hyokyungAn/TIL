...
* 가지로 분류해야 하는 이유: 여러 팀원들이 나눠서 개발할 때 별도의 브랜치를 만들어서 개발 후 기능에 문제가 없다고 판단되면 master 브랜치로 합쳐서 관리하기 위해
* 합치는 것 ➡️ Merge(병합)

<img width="1109" alt="스크린샷 2023-04-28 오전 2 47 44" src="https://user-images.githubusercontent.com/104885245/234949495-da7cf243-6777-4262-858c-480f6896cfc5.png">
...
* master라는 브랜치에서는 아무런 작업을 하지 않았고 signin에서만 작업을 했는데 signin은 다른 브랜치, 대표적으로 master에는 영향을 끼치지 않음
* 나중에 개발이 완료되면 master 브랜치로 병합을 해줌
* 병합을 하기 직전까지는 어떠한 작업을 하든지 실제 웹사이트에는 아무런 영향이 없음
